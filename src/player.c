...
void Player_Draw(const Player *p) {
  if (p->hasSprite) {
    ...
    DrawTexturePro(p->spriteSheet, sourceRect, destRect, origin, 0.0f, tint);
  } else {
    ...
    if (p->facingRight) {
      ...
      DrawTriangle((Vector2){p->position.x, p->position.y + PLAYER_SIZE / 2},
  ...
  if (p->isClinging) {
    ...
    DrawRectangle((int)p->position.x + offset, (int)p->position.y + 10, 5, 20,
                  WHITE);
  }
  DrawRectangle((int)p->position.x + 10, (int)p->position.y + 10, 8, 8, WHITE);
  DrawRectangle((int)p->position.x + 22, (int)p->position.y + 10, 8, 8, WHITE);
  DrawRectangle((int)p->position.x + 12, (int)p->position.y + 12, 4, 4, BLACK);
  DrawRectangle((int)p->position.x + 24, (int)p->position.y + 12, 4, 4, BLACK);
  ...
  DrawRectangle((int)p->position.x, (int)p->position.y - 10,
                (int)(barWidth * ((float)p->health / p->maxHealth)), barHeight,
                GREEN);
  if (p->dashCooldown > 0) {

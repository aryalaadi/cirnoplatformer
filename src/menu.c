...
void Menu_Draw(void) {
  DrawText("CIRNO'S HARDEST PARKOUR", SCREEN_WIDTH / 2 - 200, 80, 30, SKYBLUE);
  if (currentScreen == MENU_MAIN) {
    for (int i = 0; i < 6; i++) {
      Color c = (i == selectedIndex) ? YELLOW : WHITE;
      DrawText(mainMenuItems[i], SCREEN_WIDTH / 2 - 80, 200 + i * 40, 28, c);
    }
    ...
    for (int i = 0; i < 5; i++) {
      Color c = (i == selectedIndex) ? YELLOW : WHITE;
      if (i == 0) {
        DrawText(settingsItems[i], SCREEN_WIDTH / 2 - 140, 220 + i * 40, 24, c);
        DrawText(resolutionNames[settings.resolution], SCREEN_WIDTH / 2 + 80,
                 220 + i * 40, 24, c);
      } else if (i == 1) {
        DrawText(settingsItems[i], SCREEN_WIDTH / 2 - 140, 220 + i * 40, 24, c);
        DrawText(settings.fullscreen ? "ON" : "OFF", SCREEN_WIDTH / 2 + 80,
                 220 + i * 40, 24, c);
      } else if (i == 2) {
        DrawText(settingsItems[i], SCREEN_WIDTH / 2 - 140, 220 + i * 40, 24, c);
        DrawText(settings.soundEnabled ? "ON" : "OFF", SCREEN_WIDTH / 2 + 80,
                 220 + i * 40, 24, c);
      } else {
        DrawText(settingsItems[i], SCREEN_WIDTH / 2 - 140, 220 + i * 40, 24, c);
      }
    }
    DrawText("Press ENTER to toggle/select", SCREEN_WIDTH / 2 - 110, 440, 18,
             GRAY);
    ...
    if (sys) {
      ...
      DrawRectangleLinesEx((Rectangle){listX, listY, listW, listH}, 2, SKYBLUE);
      ...
      for (int i = 0; i < MAX_ACHIEVEMENTS; i++) {
        ...
        int boxH = 75;
        Color boxColor =
            ach->unlocked ? (Color){40, 60, 40, 255} : (Color){40, 40, 50, 255};
      ...
      DrawText("No save file loaded", SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2,
               24, RED);
    }
    DrawText("Use mouse wheel or arrow keys to scroll | Press ESC to go back",
             50, SCREEN_HEIGHT - 40, 16, GRAY);
  } else if (currentScreen == MENU_KEYBINDINGS) {
    DrawText("KEY BINDINGS", SCREEN_WIDTH / 2 - 110, 100, 32, SKYBLUE);
    if (waitingForKey >= 0) {
      DrawText("Press any key...", SCREEN_WIDTH / 2 - 90, 150, 24, YELLOW);
      DrawText("(ESC to cancel)", SCREEN_WIDTH / 2 - 80, 180, 18, GRAY);
    }
    for (int i = 0; i < 9; i++) {
      Color c = (i == selectedIndex && waitingForKey < 0) ? YELLOW : WHITE;
      int yPos = 220 + i * 35;
      if (i < 7) {
        DrawText(keybindingItems[i], SCREEN_WIDTH / 2 - 180, yPos, 22, c);
        const char *keyName = "";
        switch (i) {
        case 0:
        ...
        DrawText(keybindingItems[i], SCREEN_WIDTH / 2 - 100, yPos, 22, c);
      }
    }
    DrawText("Press ENTER to rebind", SCREEN_WIDTH / 2 - 100, 510, 18, GRAY);

...
// This source file is a mess, it contains the game implementation along with
// level editor.
...
#ifdef _WIN32
// TODO: ensure everything works on Linux as well
#include <direct.h>
...
void Game_Init(void)
{
	...
	// TODO: load everything from the standard program local directory instead
	// of pwd. maybe even get the path from settings so players can use their
	// own custom assets?
	mkdir("assets", 0777);
	...
	// This is basically different main menu screens for different level of
	// progression
	if (!menuBgLoaded)
	{
...
void Game_Update(void)
{
	...
	if (currentState == STATE_MENU)
	{
		...
		// If the name exists, save will be over written.
		// TODO: warn player if a save with the same name exists.
	}
	else if (currentState == STATE_NAME_INPUT)
	{
		...
		// For loading existing save files
	}
	else if (currentState == STATE_LOAD_GAME)
	{
		...
		// For cutscenes, right now, it can only be triggered at the start of a
		// level. Which can be defined from the level editor.
	}
	else if (currentState == STATE_VISUAL_NOVEL)
	{
		...
		// Once you die, theres no after life, you come to STATE_DEATH_SCREEN.
	}
	else if (currentState == STATE_DEATH_SCREEN)
	{
		...
		{
			// Longer maps will have checkpoints so players dont kill
			// themselves.
		...
		// Roll credits.
		// The credits are always there, you can just set state to roll them
		// down.
	}
		...
		if (!editor.uiInteracting)
		{
			...
			if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
			{
				    ...
				    tileY < editor.level.height)
				{
					...
					{
						editor.level.backgroundTiles[tileY][tileX] =
						    editor.selectedTile;
					}
					else
					{
						Level_SetTile(&editor.level, tileX, tileY,
						              editor.selectedTile);
					}
				}
			}
	...
	else if (currentState == STATE_EDITOR_PAUSED)
	{
		...
		// This is painful to read, it feels wrong, having level editor and game
...
void Game_Draw(void)
{
	if (currentState == STATE_MENU)
	{
		...
		if (gameData.isValid && gameData.achievements.unlockedCount > 0)
		{
			...
			else
			{
				...
				for (int i = ACH_COMPLETE_STAGE_1; i <= ACH_COMPLETE_STAGE_9;
				     i++)
				{
					if (!Achievement_IsUnlocked(&gameData.achievements,
					                            (AchievementID)i))
	...
	else if (currentState == STATE_NAME_INPUT)
	{
		...
		DrawText("CIRNO'S HARDEST PARKOUR", SCREEN_WIDTH / 2 - 250, 100, 40,
		         SKYBLUE);
		DrawText("Enter Your Name", SCREEN_WIDTH / 2 - 150,
		         SCREEN_HEIGHT / 2 - 100, 40, GOLD);
		DrawRectangle(SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2, 300, 50,
		              DARKGRAY);
		DrawRectangleLines(SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2, 300, 50,
		                   GOLD);
		DrawText(playerNameInput, SCREEN_WIDTH / 2 - 140,
		         SCREEN_HEIGHT / 2 + 10, 30, WHITE);
		if (((int)(GetTime() * 2)) % 2 == 0 && playerNameLength < 30)
		{
			DrawText("_",
			         SCREEN_WIDTH / 2 - 140 + MeasureText(playerNameInput, 30),
	...
	else if (currentState == STATE_LOAD_GAME)
	{
		...
		if (saveCount == 0)
		{
			DrawText("No save files found", SCREEN_WIDTH / 2 - 120,
		...
		else
		{
			...
			int scrollOffset =
			    (saveSlotSelection >= 6) ? (saveSlotSelection - 5) : 0;
			for (int i = scrollOffset;
			     i < scrollOffset + displayCount && i < saveCount; i++)
			{
				int yPos = startY + (i - scrollOffset) * lineHeight;
				Color boxColor = (i == saveSlotSelection)
				                     ? (Color){70, 70, 120, 255}
				                     : (Color){40, 40, 70, 255};
				Color borderColor = (i == saveSlotSelection) ? YELLOW : GRAY;
				DrawRectangle(100, yPos, SCREEN_WIDTH - 200, 70, boxColor);
				DrawRectangleLines(100, yPos, SCREEN_WIDTH - 200, 70,
				                   borderColor);
				DrawText(saves[i].saveName, 120, yPos + 10, 24, WHITE);
				DrawText(TextFormat("Level: %d", saves[i].currentLevel + 1),
				         120, yPos + 40, 18, GOLD);
				DrawText(TextFormat("Health: %d", saves[i].health), 300,
				         yPos + 40, 18, GREEN);
				DrawText(TextFormat("Deaths: %d", saves[i].deathCount), 450,
				         yPos + 40, 18, RED);
			}
			DrawText("Use UP/DOWN to select, ENTER to load",
			         SCREEN_WIDTH / 2 - 180, SCREEN_HEIGHT - 80, 20, GRAY);
			DrawText("Press ESC to go back", SCREEN_WIDTH / 2 - 100,
	...
	else if (currentState == STATE_CREDITS)
	{
		...
		y += 50;
		DrawText("Lead Developer & Artist:", SCREEN_WIDTH / 2 - 140, y, 20,
		         GRAY);
		y += 30;
		...
		y += 25;
		DrawText("Remixed: Aaditya Aryal", SCREEN_WIDTH / 2 - 100, y, 20,
		         WHITE);
		y += 60;
		...
		y += 80;
		DrawText("Thank you for playing!", SCREEN_WIDTH / 2 - 120, y, 25,
		         SKYBLUE);
		y += 50;
		DrawText("Press SPACE or ESC to return to menu", SCREEN_WIDTH / 2 - 180,
		         y, 18, GRAY);
	}
	else if (currentState == STATE_LEVEL_EDITOR ||
	         currentState == STATE_EDITOR_PAUSED)
	{
		...
		if (currentState == STATE_LEVEL_EDITOR && !editor.uiInteracting)
		{
			...
			if (tileX >= 0 && tileX < editor.level.width && tileY >= 0 &&
			    tileY < editor.level.height)
			{
				DrawRectangleLines(tileX * TILE_SIZE, tileY * TILE_SIZE,
				                   TILE_SIZE, TILE_SIZE, YELLOW);
				DrawText(TextFormat("%d", editor.selectedTile),
				         tileX * TILE_SIZE + 5, tileY * TILE_SIZE + 5, 16,
				         YELLOW);
				if (editor.selectedTile == TILE_SPAWNER_CIRCLE)
				{
					DrawCircle(tileX * TILE_SIZE + 25, tileY * TILE_SIZE + 25,
					           15, (Color){255, 0, 0, 100});
					DrawText("CIRCLE", tileX * TILE_SIZE + 5,
					         tileY * TILE_SIZE - 20, 12, RED);
				}
				else if (editor.selectedTile == TILE_SPAWNER_SPIRAL)
				{
					DrawCircle(tileX * TILE_SIZE + 25, tileY * TILE_SIZE + 25,
					           15, (Color){128, 0, 128, 100});
					DrawText("SPIRAL", tileX * TILE_SIZE + 5,
					         tileY * TILE_SIZE - 20, 12, PURPLE);
				}
				else if (editor.selectedTile == TILE_SPAWNER_WAVE)
				{
					DrawCircle(tileX * TILE_SIZE + 25, tileY * TILE_SIZE + 25,
					           15, (Color){0, 0, 255, 100});
					DrawText("WAVE", tileX * TILE_SIZE + 8,
					         tileY * TILE_SIZE - 20, 12, BLUE);
				}
				else if (editor.selectedTile == TILE_SPAWNER_BURST)
				{
					DrawCircle(tileX * TILE_SIZE + 25, tileY * TILE_SIZE + 25,
					           15, (Color){255, 165, 0, 100});
					DrawText("BURST", tileX * TILE_SIZE + 5,
					         tileY * TILE_SIZE - 20, 12, ORANGE);
				}
			}
		}
		EndMode2D();
		DrawRectangle(SCREEN_WIDTH - 260, 0, 260, SCREEN_HEIGHT,
		              (Color){30, 30, 50, 255});
		DrawLine(SCREEN_WIDTH - 260, 0, SCREEN_WIDTH - 260, SCREEN_HEIGHT,
		         YELLOW);
		int yPos = 10;
		GuiLabel((Rectangle){SCREEN_WIDTH - 250, yPos, 240, 25},
		         "LEVEL EDITOR");
		yPos += 30;
		...
		GuiLine((Rectangle){SCREEN_WIDTH - 250, yPos, 240, 1}, NULL);
		yPos += 10;
		const char *layerText = editor.editingBackground ? "Layer: BACKGROUND"
		                                                 : "Layer: FOREGROUND";
		Color layerColor = editor.editingBackground ? SKYBLUE : YELLOW;
		DrawText(layerText, SCREEN_WIDTH - 245, yPos, 16, layerColor);
		DrawText("(Press B to toggle)", SCREEN_WIDTH - 245, yPos + 18, 12,
		         GRAY);
		yPos += 45;
		GuiLine((Rectangle){SCREEN_WIDTH - 250, yPos, 240, 1}, NULL);
		yPos += 10;
		...
		DrawRectangleLinesEx(
		    ...
		    DARKGRAY);
		Vector2 mousePos = GetMousePosition();
		Rectangle paletteRect = {paletteX, paletteStartY, paletteW,
		                         paletteHeight};
		...
		int tileSpacing = 5;
		int buttonSize =
		    (paletteW - (tilesPerRow + 1) * tileSpacing) / tilesPerRow;
		...
		for (int i = 0; i < 20; i++)
		{
			...
			int btnX =
			    paletteX + tileSpacing + col * (buttonSize + tileSpacing);
			    ...
			    btnY < paletteStartY + paletteHeight)
			{
				...
				Color btnColor = isSelected ? (Color){80, 80, 120, 255}
				                            : (Color){40, 40, 60, 255};
				if (isHovered)
		...
		for (int i = 0; i < 80; i++)
		{
			    ...
			    btnY < paletteStartY + paletteHeight)
			{
				...
				Color btnColor = isSelected ? (Color){80, 120, 120, 255}
				                            : (Color){40, 60, 60, 255};
				if (isHovered)
				...
				DrawText(TextFormat("%d", tileId), btnX + 3, btnY + 3, 10,
				         SKYBLUE);
				if (isHovered)
				{
					hoveredTileRect = btnRect;
		...
		{
			int tooltipX =
			    (int)hoveredTileRect.x + (int)hoveredTileRect.width + 5;
			...
			DrawRectangle(tooltipX, tooltipY, tooltipW, tooltipH,
			              (Color){0, 0, 0, 230});
			DrawRectangleLinesEx(
			    (Rectangle){tooltipX, tooltipY, tooltipW, tooltipH}, 1, GOLD);
			DrawText(hoveredTileName, tooltipX + 5, tooltipY + 4, 14, WHITE);
		}
		int maxScroll =
		    ((19 + 80) / tilesPerRow + 3) * (buttonSize + tileSpacing) + 40 -
		    paletteHeight;
		if (maxScroll > 0)
		{
			...
			DrawRectangle(paletteX + paletteW - 8, scrollbarY, 6,
			              scrollbarHeight, GOLD);
		}
		...
		if (editor.vnEditorOpen)
		{
			...
			DrawText("ESC to close", panelX + panelW - 120, panelY + 20, 14,
			         GRAY);
			int y = panelY + 55;
			...
			if (editor.level.hasVisualNovel)
			{
				...
				int listX = panelX + 20;
				int listW = 220;
				DrawText(TextFormat("Dialogues (%d/20):",
				...
				DrawRectangleLinesEx((Rectangle){listX, y, listW, 320}, 2,
				                     DARKGRAY);
				for (int i = 0; i < editor.level.dialogueCount; i++)
				{
					Color btnColor = (i == editor.selectedDialogue)
					                     ? (Color){60, 60, 80, 255}
					                     : (Color){30, 30, 45, 255};
					Rectangle btnRect = {listX + 5, y + 5 + i * 32, listW - 10,
					                     28};
					DrawRectangleRec(btnRect, btnColor);
					    ...
					    IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
					{
						...
						strcpy(editor.vnTextBuffer,
						       editor.level.dialogues[i].text);
						strcpy(editor.vnSpriteBuffer,
						       editor.level.dialogues[i].characterSprite);
						editor.vnBgColorIndex =
						    editor.level.dialogues[i].bgColor;
						editor.vnEditingCharName = false;
						editor.vnEditingText = false;
						editor.vnEditingSprite = false;
					}
				    ...
				    editor.selectedDialogue < editor.level.dialogueCount)
				{
					...
					int editY = y;
					int editW = 400;
					DrawText(TextFormat("Editing Dialogue #%d",
					                    editor.selectedDialogue + 1),
					...
					DrawRectangleLinesEx(nameBox, 2,
					                     editor.vnEditingCharName ? SKYBLUE
					                                              : DARKGRAY);
					...
					if (editor.vnEditingCharName)
					{
						...
						while (key > 0)
						{
							    ...
							    strlen(editor.vnCharNameBuffer) < 30)
							{
								...
								strcpy(editor.level
								           .dialogues[editor.selectedDialogue]
								           .characterName,
						...
						{
							editor.vnCharNameBuffer
							    [strlen(editor.vnCharNameBuffer) - 1] = '\0';
							strcpy(
							    editor.level.dialogues[editor.selectedDialogue]
							        .characterName,
							    editor.vnCharNameBuffer);
					...
					DrawText(editor.vnCharNameBuffer, editX + 5, editY + 8, 16,
					         WHITE);
					if (editor.vnEditingCharName &&
					    ((int)(GetTime() * 2) % 2 == 0))
					{
						DrawText("_",
						         editX + 5 +
						             MeasureText(editor.vnCharNameBuffer, 16),
						         editY + 8, 16, YELLOW);
					}
					editY += 38;
					...
					DrawRectangleRec(textBox, editor.vnEditingText
					                              ...
					                              : (Color){30, 30, 45, 255});
					DrawRectangleLinesEx(
					    textBox, 2, editor.vnEditingText ? SKYBLUE : DARKGRAY);
					...
					if (editor.vnEditingText)
					{
						...
						{
							editor
							    .vnTextBuffer[strlen(editor.vnTextBuffer) - 1] =
							    '\0';
							strcpy(
							    editor.level.dialogues[editor.selectedDialogue]
							        .text,
						...
						if (IsKeyPressed(KEY_ENTER) &&
						    strlen(editor.vnTextBuffer) < 253)
						{
					...
					DrawText(editor.vnTextBuffer, editX + 5, editY + 5, 14,
					         WHITE);
					if (editor.vnEditingText && ((int)(GetTime() * 2) % 2 == 0))
					{
						DrawText("_",
						         editX + 5 +
						             MeasureText(editor.vnTextBuffer, 14),
						         editY + 5, 14, YELLOW);
					}
					editY += 88;
					DrawText("Sprite Path (optional):", editX, editY, 15,
					         WHITE);
					editY += 20;
					...
					DrawRectangleLinesEx(spriteBox, 2,
					                     editor.vnEditingSprite ? SKYBLUE
					                                            : DARKGRAY);
					...
					if (editor.vnEditingSprite)
					{
						...
						{
							editor
							    .vnSpriteBuffer[strlen(editor.vnSpriteBuffer) -
							                    1] = '\0';
							strcpy(
							    editor.level.dialogues[editor.selectedDialogue]
							        .characterSprite,
					    ...
					    ((int)(GetTime() * 2) % 2 == 0))
					{
						DrawText("_",
						         editX + 5 +
						             MeasureText(editor.vnSpriteBuffer, 14),
						         editY + 6, 14, YELLOW);
					}
					editY += 35;
					DrawText("Background:", editX, editY, 15, WHITE);
					const char *colorNames[] = {"Dark Blue", "Dark Red",
					                            "Dark Green", "Gray", "Black"};
					DrawText(colorNames[editor.vnBgColorIndex], editX + 110,
					         editY, 15, SKYBLUE);
					if (GuiButton((Rectangle){editX + 230, editY - 3, 30, 25},
					              "<"))
					{
						editor.vnBgColorIndex =
						    (editor.vnBgColorIndex - 1 + 5) % 5;
						editor.level.dialogues[editor.selectedDialogue]
						    .bgColor = editor.vnBgColorIndex;
					}
					              ...
					              "Delete Dialogue"))
					{
						...
						{
							editor.level.dialogues[i] =
							    editor.level.dialogues[i + 1];
						}
						editor.level.dialogueCount--;
						if (editor.selectedDialogue >=
						    editor.level.dialogueCount)
						{
							editor.selectedDialogue =
							    editor.level.dialogueCount - 1;
						}
						if (editor.selectedDialogue >= 0)
						{
							strcpy(
				...
				DrawText(
				    "Click text boxes to edit | This VN plays BEFORE the level",
	...
	else if (currentState == STATE_LEVEL_COMPLETE)
	{
		...
		DrawText("LEVEL COMPLETE!", SCREEN_WIDTH / 2 - 140,
	...
	{
		ClearBackground((Color){20, 20, 40, 255});
		DrawText("CONGRATULATIONS!", SCREEN_WIDTH / 2 - 160,
		         SCREEN_HEIGHT / 2 - 80, 40, GOLD);
	...
	if (saveMenuState != SAVE_MENU_NONE)
	{
		...
		for (int i = 0; i < MAX_SAVE_SLOTS; i++)
		{
			...
			{
				DrawText(TextFormat("[%d] %s - Level %d", i + 1,
				                    saveSlots[i].saveName,
				                    saveSlots[i].currentLevel + 1),
				         SCREEN_WIDTH / 2 - 150, y, 20, c);
			}
			else
			{
				DrawText(TextFormat("[%d] Empty Slot", i + 1),
				         SCREEN_WIDTH / 2 - 150, y, 20, c);
			}
		}
	...
	if (achievementNotifTimer > 0 && currentState != STATE_MENU)
	{
		...
		if (ach)
		{
			...
			Color borderColor =
			    (Color){255, 215, 0, (unsigned char)(255 * alpha)};
			Color textColor =
